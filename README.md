...
1. What is the purpose of using _sessions_?
    ...
    Sessions are used to allow a server to store information about a client. In the carriculum for this week, we used it so persist authentication data, which allows the user to not have to login on every visit.

2. What does bcrypt do to help us store passwords in a secure manner.

    Bcrypt is a third part module, when added provied a way for back-end builders to hash passwords.

3. What does bcrypt do to slow down attackers?

    The accumulative hashing rounds, means the attacker would need to know the number of rounds used to hash in addition to the algoritm used, which in turn would slow down an attack.
